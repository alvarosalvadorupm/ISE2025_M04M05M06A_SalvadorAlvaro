...
//    GLCD_DrawString (x*16U, 4U*24U, buf);
    ...
    //sprintf (buf, "%-20s", lcd_text[0]);		// Obtiene los primeros 20 caracteres del array lcd_text[0] y se almacenan en buf.
		write_lcd(lcd_text[0],1,strlen(lcd_text[0]));
		//Linea 2 LCD
    //sprintf (buf, "%-20s", lcd_text[1]);		// Obtiene los primeros 20 caracteres del array lcd_text[1] y se almacenan en buf.
