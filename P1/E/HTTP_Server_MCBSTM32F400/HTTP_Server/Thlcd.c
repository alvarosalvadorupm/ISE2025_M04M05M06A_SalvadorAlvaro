...
// Funcion en la que pintamos en la linea 1 del LCD gestionando si el ultimo caracter a representar entra entero o no
void symbolToLocalBuffer_L1(uint8_t symbol){ //Escribe en la página 0 y 1, eso es la línea 1
  ...
  if(positionL1 + Arial12x12[offset] <= 127){   // Condicion para comprobar si el ultimo caracter a representar en el LCD entra entero y pintarlo o no
    for (i = 0; i < 12; i++){
...
void symbolToLocalBuffer_L2(uint8_t symbol){ //Escribe en la página 2 y 3, eso es la línea 2
  ...
  if(positionL2 + Arial12x12[offset] <= 127){   // Condicion para comprobar si el ultimo caracter a representar en el LCD entra entero y pintarlo o no
    for (i = 0; i < 12; i++){
...
void pintar_LCD(char cadena[], int linea){
  ...
  switch(linea){
    ...
    case 1:
      for(int j = 0; j < cadena[j] != '\0'; j++){
        symbolToLocalBuffer_L1(cadena[j]);
      }
    break;
    
    case 2:
      for(int j = 0; j < cadena[j] != '\0'; j++){
        symbolToLocalBuffer_L2(cadena[j]);
      }
    break;
  }
